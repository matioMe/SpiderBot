...
int Leg::calcAnglesServos(Point target) //Point target //m_actual
{
    ...
    //calc position of A point in x/z plan OK
    xA=m_point_A.x;
    yA=m_point_A.y;

    //calc length leg on x/y plan with pythagore
    l=sqrt((m_actual.x-xA)*(m_actual.x-xA)+(m_actual.y-yA)*(m_actual.y-yA)); //OK

    //A angle in x/y plan
    //quadrant haut droit
    if(m_actual.y >= yA && m_actual.x > xA)
    {
    ...
    if(m_actual.y > yA && m_actual.x < xA)
    {
        m_Ar=asin((xA-m_actual.x)/l)+PI/2;
    }
    //quadrant bas gauche
    if(m_actual.y <= yA && m_actual.x < xA)
    {
        m_Ar=acos((xA-m_actual.x)/l)+PI;
    }
    //quadrant bas droit
    if(m_actual.y < yA && m_actual.x > xA)
    {
        m_Ar=asin((m_actual.x-xA)/l)+6*PI/4;
    }

    //calc of the [BD] length in local u/v plan
    e=sqrt((l-m_Al)*(l-m_Al)+m_actual.z*m_actual.z); //OK

    //angle of (vBD) in u/v plan
    mu=acos((l-m_Al)/e);

    if(m_actual.z<0) mu=-mu;

    //intermediate values for last angles calc OK
    U=(m_Bl*m_Bl+e*e-m_Cl*m_Cl)/(2*e);
    V=e-U;

    //final angles calc
    m_Br=acos(U/m_Bl)+mu;
    m_Cr=mu-acos(V/m_Cl)-m_Br;

    //conversion of attributes in degrees
    m_Ad = (m_Ar*180/PI)-m_Od+90; //OK
    m_Bd = 90-m_Br*180/PI; //
    m_Cd = m_Cr*180/PI+90; //
...
int Leg::setLengh(float o, float a, float b, float c)
{
    m_Ol=o;
    m_Al=a;
    m_Bl=b;
...
void Leg::SA_calcStep(Point P_b, int div, float h)
{
    m_start=m_actual;
    m_end=P_b;
    m_div=div;
    ...
    m_SA_const_move1=(m_end.x-m_start.x)/m_div;
    m_SA_const_move2=(m_end.y-m_start.y)/m_div;

    m_actual.z+=h;

}

Point Leg::SA_moveStep(int n)
{
...
int Leg::calcLegMove(Point P_b, int div)
{
    //transfert of values
    m_start=m_actual;
    m_end=P_b;
...
int Leg::calcLegStepRegular(Point P_b, int div, float h)
{
    m_start=m_actual;
    m_end=P_b;
...
Point Leg::moveLegStepRegular(int n) //gérer les arrondis
{
    ...
    m_actual.z=m_const_step1*p*p+m_const_step2*p+m_start.z;

    if(n==m_div) //round for the last value
    {
...
/*
int Leg::calcLegStepAccelerate(Point P_b, int div, float h)
{
    m_start=m_actual;
    m_end=P_b;
    m_div=div; //peut-être à mettre au constructeur et généraliser le tout avec un #define
