...
public:
    ...
    //Deplacement
    int calcAnglesServos(Point target);

    int calcLegStepAccelerate(Point P_b, int div, float h);
    int calcLegStepRegular(Point P_b, int div, float h);
    int calcLegMove(Point P_b, int div);
    Point moveLegStepAccelerate(int n);
    Point moveLegMove(int n);
    Point moveLegStepRegular(int n);

    //setup + debug
    int setLengh(float o, float a, float b, float c);
    int setOd(float Od);
    int setActual(Point P);
    void printActual();
    void printData();

    //save oral
    void SA_calcStep(Point P_b, int div, float h);
    Point SA_moveStep(int n);
...
private:
    ...
    float m_SA_const_move1, m_SA_const_move2;
